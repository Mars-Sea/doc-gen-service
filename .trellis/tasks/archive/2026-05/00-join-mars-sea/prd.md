...
`Mars-Sea` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Mars-Sea/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Mars-Sea/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Mars-Sea` to
...
python ./.trellis/scripts/task.py archive 00-join-mars-sea
```
