 ...
 * 文档生成 API 控制器
 *
 * <p>提供 RESTful API 接口，支持根据模板和数据动态生成 Word 和 Excel 文档。 生成的文档以二进制流形式返回，可直接被客户端下载。
 *
 * <p>接口路径: /api/v1/doc
 *
 * @author Mars-Sea
...
public class DocController {
     ...
     * 生成 Word 文档
     *
     * <p>根据指定的模板名称和渲染数据，生成 Word 文档并返回文件流。 支持自定义输出文件名，包括中文文件名。
     *
    ...
    @PostMapping("/word")
    public ResponseEntity<byte[]> generateWord(@Valid @RequestBody WordGenRequest request)
            throws IOException {
        log.info(
                "Received word generation request, template: {}, fileName: {}",
        ...
        log.info(
                "Word document generated successfully, size: {} bytes, fileName: {}",
                bytes.length,
                fileName);
    ...
    public ResponseEntity<byte[]> batchGenerateWord(@Valid @RequestBody WordBatchRequest request)
            throws IOException {
        log.info(
                "Received batch word generation request, template: {}, dataCount: {}, fileName: {}",
                request.getTemplateName(),
     ...
     * 生成 Excel 文档
     *
     * <p>根据指定的表头和数据，动态生成 Excel 文档并返回文件流。 支持自定义输出文件名，包括中文文件名。
     *
    ...
    public ResponseEntity<byte[]> generateExcel(@Valid @RequestBody ExcelGenRequest request) {
        log.info(
                "Received excel generation request, sheetName: {}, columns: {}, rows: {}, fileName:"
                        + " {}",
        ...
        byte[] bytes =
                excelService.generateExcel(
                        request.getSheetName(), request.getHeaders(), request.getData());
        ...
        String encodedFileName =
                URLEncoder.encode(fileName + ".xlsx", StandardCharsets.UTF_8).replace("+", "%20");

        log.info(
                "Excel document generated successfully, size: {} bytes, fileName: {}",
                bytes.length,
                fileName);
     ...
     * 基于模板填充 Excel 文档
     *
     * <p>根据指定的模板和数据，填充生成 Excel 文档并返回文件流。 支持单值变量替换和列表数据循环填充。
     *
     * @param request 包含模板名称、单值数据、列表数据和可选文件名的请求体
     * @return 生成的 .xlsx 文件二进制流
     */
    @Operation(
            summary = "基于模板填充 Excel 文档",
            description = "根据 Excel 模板和数据生成文档。支持 {variable} 单值替换和 {.field} 列表循环。")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<byte[]> fillExcelTemplate(@Valid @RequestBody ExcelFillRequest request) {
        ...
        log.info(
                "Excel template filled successfully, size: {} bytes, fileName: {}",
                bytes.length,
                fileName);
