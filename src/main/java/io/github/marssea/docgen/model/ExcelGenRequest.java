 ...
 * Excel 文档生成请求体 DTO
 *
 * <p>定义客户端请求生成 Excel 文档时需要传递的参数结构。 支持动态表头和二维数据填充。
 *
 * <h3>请求示例:</h3>
 *
 * <pre>
...
public class ExcelGenRequest {
     ...
     * 数据行列表
     *
     * <p>二维列表，每个内层列表代表一行数据， 列顺序需与 headers 对应
     */
    @NotEmpty(message = "数据列表不能为空")
    @Schema(
            description = "数据行（二维数组，每行对应 headers 顺序）",
            example = "[[\"张三\", 25, \"北京\"], [\"李四\", 30, \"上海\"]]",
            requiredMode = Schema.RequiredMode.REQUIRED)
     ...
     * 自定义输出文件名（可选）
     *
     * <p>不含扩展名，系统会自动添加 .xlsx 后缀。 如不指定，则默认使用 "generated"。
     */
