 ...
 * 批量 Word 文档生成请求体 DTO
 *
 * <p>定义客户端请求批量生成 Word 文档时需要传递的参数结构。 使用同一模板渲染多条数据，每条数据生成一份完整模板实例，合并为单个文档。
 *
 * <h3>请求示例:</h3>
 *
 * <pre>
...
public class WordBatchRequest {
     ...
     * 模板文件名称
     *
     * <p>需包含扩展名（如 certificate.docx）。 该文件必须存在于配置的 templatePath 目录下。
     */
     ...
     * 数据列表
     *
     * <p>每个 Map 对应一份完整模板实例的渲染数据，Key 对应模板中的占位符。 列表中有多少条数据，生成的文档就包含多少份模板实例。
     *
    ...
    @Schema(
            description = "数据列表，每条数据生成一份完整模板实例。支持图片载荷（type: image）。",
            example =
     ...
     * 自定义输出文件名（可选）
     *
     * <p>不含扩展名，系统会自动添加 .docx 后缀。 如不指定，则默认使用 "batch_generated"。
     */
