 ...
 * Word 文档生成请求体 DTO
 *
 * <p>定义客户端请求生成 Word 文档时需要传递的参数结构。 该类用于接收来自调用方（如 Go 服务）的 JSON 数据。
 *
 * <h3>请求示例:</h3>
 *
 * <pre>
...
public class WordGenRequest {
     ...
     * 模板文件名称
     *
     * <p>需包含扩展名（如 template.docx）。 该文件必须存在于配置的 templatePath 目录下。
     */
    @NotBlank(message = "模板文件名不能为空")
    @Schema(
            description = "模板文件名（需包含扩展名）",
            example = "test-template.docx",
     ...
     * 填充到模板中的动态数据
     *
     * <p>Key 对应模板中的占位符标签（如 {{title}}）， Value 为具体的填充值，支持以下类型：
     *
     * <ul>
     ...
     * 自定义输出文件名（可选）
     *
     * <p>不含扩展名，系统会自动添加 .docx 后缀。 支持中文等非 ASCII 字符。 如不指定，则默认使用 "generated"。
     */
